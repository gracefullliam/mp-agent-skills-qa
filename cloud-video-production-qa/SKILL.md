...
- 用户对已有成片要求“换音乐”时，继续原会话提交原话。当前服务端会通过重新选择模板生成新成片，不是在原成片上单独替换音轨；不得向用户宣称“已单独换音乐”。
- 用户对已有成片要求移动、交换或调整素材顺序时，继续原会话提交原话。当前服务端会基于现有素材重新成片，但不能保证顺序完全按用户要求生效；不得承诺精确重排。
- 上述非核心编辑请求不得由 Skill 本地修改视频、音轨或素材顺序，也不得新建会话绕过服务端上下文。服务端返回成片后，必须展示其能力边界说明并请用户确认效果；后续能力随 Agent 和组织逻辑版本迭代。
...
普通成片完成且 `feedback.message` 为空：只返回“成片创作完成”和视频下载/预览链接。

成片完成且 `feedback.message` 非空：先返回“成片创作完成”和视频链接，再单独展示服务端给出的能力边界说明并请用户确认效果。不要把 `current_node_description` 与 `feedback.message` 机械拼接，也不要省略非空反馈。

- 换音乐的反馈必须忠实表达“当前不支持单独替换音乐，本次通过更换模板重新成片，音乐和画面效果可能一起变化”。
- 调整素材顺序的反馈必须忠实表达“当前不支持精确重排，本次为 Best-effort 重新成片，顺序不保证完全生效”。
- 可以说明后续随 Agent 或组织逻辑版本迭代支持更精细编辑，但不得承诺具体上线时间。
