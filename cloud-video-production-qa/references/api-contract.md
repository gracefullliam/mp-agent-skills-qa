...
| POST | `/api/rest/mva/out/cloud/cancel` | Cancel the current active Cloud production Command | Cancels queued/dispatching/running Jobs and the active internal Turn; does not delete completed videos |
...
Poll every 3–5 seconds. Stop after a terminal state. Stopping a local wait does not cancel server work; call `/cancel` only when the user explicitly requests cancellation.
...
### Batch production progress and completion

A natural-language request asking several shown templates to produce one video each is sent once to `/make`. The service binds the user's references to the fixed recommendation snapshot and executes ordered Jobs inside the same public conversation. Do not split the request into multiple `/make` calls.

While the Command is active, Poll returns aggregate progress:
...
```json
{
  ...
  "data": {
    ...
    "current_node_description": "正在制作 2/3 个成片",
    "error_messages": [],
    "completed_count": 1,
    "total_count": 3
  },
...
At terminal state, read `outcome=videos` and preserve the returned order:
...
```json
{
  ...
  "data": {
    ...
    "current_node": "partial_failed",
    "current_node_description": "部分成片创作失败",
    ...
    "video_results": [
      ...
      {
        "sequence_index": 2,
        "template_code": "0x010000000000253D",
        "title": "全力以赴的快乐",
        "status": "failed",
        "video_url": "",
...
`partial_failed` is still returned as a successful HTTP response so completed videos remain usable. Display `title`, per-item status, and non-empty `video_url`; keep `template_code` and internal identifiers out of normal user replies.
...
Call cancellation only after an explicit user request:

```http
POST /api/rest/mva/out/cloud/cancel
```
...
The service cancels queued, dispatching, and running Jobs and asks the current internal Task to cancel. Completed videos are not deleted. If no active work exists, the API returns `409106`; do not report a successful cancellation and do not retry indefinitely.
