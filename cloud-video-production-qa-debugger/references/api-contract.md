...
When `outcome=recommendation` and `execution_readiness=NEED_USER_INPUT`, present `feedback`, `recommendations`, and `template_recommendations`, then forward the user's next message unchanged. If the user asks for more templates or says the candidates are unsatisfactory, submit that original text as another Turn with the fixed `conversation_id`, no `assets`, and a new `outer_request_id`; do not select or fabricate candidates locally. The service may classify this as `request_more_templates` and use its recommendation cache to exclude already exposed templates. For user-facing rendering, show each returned template as a compact item with its title and a clickable preview link. The service field `preview_url` (or an internal `previewUrl` mapping) is only the source URL; never use `preview_url` or `previewUrl` as visible link text or a label. The visible Markdown link text must be exactly “点击查看模板效果”. Show only the service-returned batch, normally 3–5 items, and never expose `template_code`, internal scores, database fields, `preview_url`, or `previewUrl` labels unless the operator explicitly asks for raw diagnostics. If `preview_url` is empty, show “暂无预览” and never fabricate a URL.
